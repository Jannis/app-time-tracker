...
class BaseDir:
    ...
    def get_data_home(self):
        if self.is_valid("XDG_DATA_HOME"):
            return environ.get("XDG_DATA_HOME")
        else:
            return path.join(self.get_home(), ".local", "share")

    def get_config_home(self):
        if self.is_valid("XDG_CONFIG_HOME"):
            return environ.get("XDG_CONFIG_HOME")
        else:
            return path.join(self.get_home(), ".config")

    def get_data_dirs(self):
        if self.is_valid("XDG_DATA_DIRS"):
    ...
    def get_config_dirs(self):
        if self.is_valid("XDG_CONFIG_DIRS"):
            return environ.get("XDG_CONFIG_DIRS").split(":")
        else:
            return [path.join(self.get_sysconf_dir(), "xdg")]

    def get_home(self):
        if self.is_valid("HOME"):
